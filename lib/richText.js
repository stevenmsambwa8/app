...
// - #hashtags and numbers are bolded + coloured
...
// .rich-number classes (defined in globals.css) so every screen that
...
export function richTextHtml(text) {  const escaped = escapeHtml(text);
  let out = '';
  let lastIndex = 0;
  ...
  for (const m of escaped.matchAll(RICH_RE)) {
    ...
    if (url) {
      ...
      out += `<strong class="rich-number">${number}</strong>`;
    }
