...
// - #hashtags are bolded + coloured + linked to /tag/<hashtag>
...
export function richTextHtml(text) {
  ...
  for (const m of escaped.matchAll(RICH_RE)) {
    ...
    if (url) {
      ...
      out += `<a href="/tag/${hashtag.slice(1)}" class="rich-hashtag">${hashtag}</a>`;
    } else if (phone) {
      out += `<a href="tel:${phone}" class="rich-phone" data-phone-confirm="${phone}">${phone}</a>`;
    }
